    ...
    <div class="w-full max-w-sm">
      ...
      <div class="card p-6 space-y-5">
        ...
        <form class="space-y-4" @submit.prevent="handleLogin">
          <BaseInput v-model="form.email" label="Correo electrónico" type="email" placeholder="tu@email.com" required />
          <BaseInput v-model="form.password" label="Contraseña" type="password" placeholder="••••••••" required />
          <BaseButton type="submit" class="w-full" :loading="loading">Iniciar sesión</BaseButton>
        </form>
      </div>
    </div>
  </div>
